...
const ChainedAttackPage = () => {
  ...
  const capture = (e: React.FormEvent) => {
    ...
    setStep("captured");
    setTimeout(() => {
      push("[3/3] 의심 회피용으로 진짜 알렌 노트 페이지로 top redirect");
      try {
  ...
  return (
    <>
      ...
      <ol>
        <li>
          iframe 임베드 후 즉시 풀스크린 오버레이로 알렌 UI 위장 (사용자는
          여전히 알렌 사이트라고 인지).
        </li>
        <li>자격증명 입력 → 자기 origin 으로 전송 → attacker 서버 도착.</li>
        <li>
          전송 직후 top redirect 로 진짜 알렌 페이지로 보내서 의심 회피. 사용자
      ...
      <ul>
        ...
        <li>
          1단계만 막아도 체인이 끊김 → host 화이트리스트 또는 sandbox 의 적절한
